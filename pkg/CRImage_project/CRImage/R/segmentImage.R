...
function(filename="",image=NA){
	...
	#	}	
		...
		imgGt=imgG
		imgGt[failures]=2
		imgGt=imgGt[-indexWhitePixel]
		imgGt=imgGt[imgGt != 2]
		imgGt=imgGt[imgGt != 1]
		globalThreshold=calculateThreshold(as.vector(imgGt))
	
		rm(imgGt)
		print("trehsold calculated")
		print(globalThreshold)
		imgB[imgG<globalThreshold]=-1
		imgB[imgB != -1]=0
		imgB[imgB==-1]=1
		...
		a=array(numSeq[imgSdN]<40,dim(imgSdN))
		imgSdN[a]=1
		imgSdN=imgSdN-1
