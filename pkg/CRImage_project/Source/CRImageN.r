...
segmentImage=function(filename="",image=NA){
	failureImage=FALSE
	if(filename!=""){
	...
	for(iW in 1:numWindows){
		...
		if(length(regions>0)){
				...
				cellCoordinates=round(allFeatures[,c("g.x","g.y")],digits=3)
				...
				print(max(cellCoordinates[,"g.x"]))
				print(max(cellCoordinates[,"g.y"]))
				print(dim(cellCoordinates))
				...
				l=list(img,imgW,allFeatures,indexWhitePixel)
				
			}else{
				print("no cells")
				allFeatures=data.frame()
				l=list(img,img,allFeatures,indexWhitePixel)
		}
...
classifyCells=function(classifier,filename="",image=NA,segmentedImage=NA,featuresObjects=NA,paint=TRUE,KS=FALSE){
	print("classify cell")
	if(filename != ""){
	...
	if(length(classLabels) >2){
		print("More than two classes, Kernel Smoother is not applied.")
			...
			print(allNewClasses)
			print(classLabels)
			indexFirstClass=which(allNewClasses[,"classValues"]<0.5)
			print(indexFirstClass)
			indexSecondClass=which(allNewClasses[,"classValues"]>=0.5)
			print(indexSecondClass)
			allNewClasses$classValues[indexFirstClass]=classLabels[1]
			allNewClasses$classValues[indexSecondClass]=classLabels[2]
...
paintCells=function(imgWT,img,classes,index,classValues){
	if(length(classValues)<=10){
		...
		imgOld=img
		cols=c("red","green","yellow","orange","blue","brown","cyan","gray","purple","violet")
		counter=1
...
calculateCellularity=function(filename="",image=NA,classifier,cancerIdentifier,KS=FALSE){
	print(filename)
	...
	print(indexWhitePixel)
	classes=classifyCells(classifier,"",img,imgW,cellData,paint=TRUE,KS=KS)
	print(classes)
...
determineCellularity=function(classes,classifiedCells,dimImg,img,imgW,indexWhitePixel,cancerIdentifier){
	print(classes)
